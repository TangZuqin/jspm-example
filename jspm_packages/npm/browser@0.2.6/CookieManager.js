...
CookieManager.prototype.get = function(url) {
  ...
  var vals = canditates.reduce(function(sofar, cookieInfo) {
    if (cookieInfo.HttpOnly && !isHTTP)
      return sofar;
    if (cookieInfo.Secure && !isSecure)
      return sofar;
    if (cookieInfo.Expires) {
      if (typeof cookieInfo.Expires == "string")
        cookieInfo.Expires = new Date(cookieInfo.Expires);
      if (cookieInfo.Expires.getTime() < time)
        return sofar;
    }
    if (cookieInfo.Path && path.indexOf(cookieInfo.Path) != 0)
      return sofar;
...
function getCookieList(hostname) {
  return hostname.split('.').reverse().reduce(function(sofar, dom) {
    ...
    if (sofar.current.$)
      sofar.current.$.forEach(function(v) {
        sofar.arr.push(v);
      });
    return sofar;
  }, {
