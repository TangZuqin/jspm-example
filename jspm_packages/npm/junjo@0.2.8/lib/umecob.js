...
(function(Buffer, process) {
  var umecob = (function() {
    ...
    $U('startValidator', function(params) {
      ...
      return params.starts.filter(function(fn) {
    ...
    $U('inputValidator', function() {
      ...
      ['tpl_getter', 'data_getter', 'compiler'].forEach(function(k) {
        if (typeof params[k] == 'object') {} else if (typeof params[k] == 'string' && umecob[k + 's'][params[k]]) {
    ...
    }).eshift().after('inputValidator').fail(function(e, args) {
      this.out = e.message;
      this.terminate();
    ...
    $U('data_getter', function(params) {
      ...
      if (typeof attach == 'object')
        for (var i in attach)
          data[i] = (data[i] === undefined) ? attach[i] : data[i];
    ...
    $U('render', function() {
      ...
      if (r.constructor == Junjo)
        ...
        return r;
    }).eshift().after('compile', 'data').next('ends', function(result) {
      var params = this.shared.params;
      params.result = result;
      var params = this.shared.params;
      params.ends.forEach(function(fn) {
        fn.apply(params);
      });
      this.out = params.result;
    });
    ...
    if (umecob.node) {
      ...
      var Request = function() {
        return typeof(ActiveXObject) !== "undefined" ? new ActiveXObject("Msxml2.XMLHTTP") || new ActiveXObject("Microsoft.XMLHTTP") : new XMLHttpRequest();
      };
      getters.ajax = {
        sync: function(id) {
    ...
    var uBuffer = function() {
      this.init();
    };
    uBuffer.prototype.init = function(i, c) {
      this.i = 0;
      this.arr = new Array();
    };
    uBuffer.prototype.add = function(c) {
      this.arr[this.i++] = c;
    };
    uBuffer.prototype.push = uBuffer.prototype.add;
    uBuffer.prototype.pop = function() {
      this.i--;
      return this.arr.pop();
    };
    uBuffer.prototype.join = function(sep) {
      return this.arr.join(sep || '');
    };
    uBuffer.prototype.getIndex = function() {
      return this.i;
    };
    uBuffer.prototype.increment = function() {
      this.i++;
    };
    uBuffer.prototype.get = function(i) {
      return this.arr[i];
    };
    uBuffer.prototype.put = function(i, c) {
      this.arr[i] = c;
    };
    ...
    umecob.render = function(params) {
      ...
      echo.addJunjo = function(fn) {
        $j.register(buff.getIndex(), fn);
        buff.increment();
      };
      echo.addUmecob = function(um) {
        (Junjo.isJunjo(um)) ? echo.addJunjo(um) : echo(um);
      };
      ...
      try {
        ...
        var tplname = params.tpl_id || ((typeof params.tpl == "string") ? "template :  >>> " + params.tpl.substr(0, 50).replace(/\n/g, " ") + "...  " : "compiled code: >>> " + params.code.substr(76, 50).replace(/\n/g, " "));
    ...
    Parser.prototype[JS_PRE_ENDMCOMMENT] = function(c) {
      ...
      switch (c) {
        case '/':
          return this.stack.pop();
        default:
          return Parser.prototype[JS_MCOMMENT].call(this, c);
      }
    };
